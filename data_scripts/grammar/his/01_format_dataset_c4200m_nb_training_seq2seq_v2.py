...
if __name__ == '__main__':
    ...
    for candidate in candidates:
        label = (f'There is {len(candidate.updates)} error{"s" if len(candidate.updates) > 1 else ""}, '
                 f'that {"are" if len(candidate.updates) > 1 else "is"}: ')
        for i, update in enumerate(candidate.updates):
            label += f'{i+1}, word <{update["rp_to"]}> should be <{update["rp_from"]}>. '
        label += 'And the correct text should be: ' + candidate.target_sent
        data = {
    ...
    output_dir = '/mnt/nlp/xingguang/llama/datasets/nb_training/grammar_c4200m_seq2seq_v2'
